...
def _safely_decode_text(file_bytes: bytes, filename: str, content_type: str) -> str:
    ...
    if filename.endswith('.pdf') or content_type == "application/pdf":
        ...
        logger.info("Plain text file detected. Attempting UTF-8 decode with error replacement...")
        # Directly decode with utf-8, replacing any invalid characters
        text = file_bytes.decode('utf-8', errors='replace')
        logger.info("Text file successfully decoded with UTF-8 (invalid characters replaced).")
    else:
